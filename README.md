...
5. In your browser go to SURVEY_ENVIRONMENT_URL/admin. 
6. Enter the credentials for your survey application. The username is fixed to 'admin'.
   The admin password is generated by AWS Secrets Manager for each SurvaasClusterStack.
   Retrieve the password from the corresponding secret key `adminPassword` in AWS Secrets Manager.
...
The loadbalancer is created once in the SurvaasRootStack and shared across all SurvaasClusterStacks.
Traffic is routed via explicit host-header listener rules and unmatched traffic returns HTTP 404.
...
Each SurvaasClusterStack also creates one dedicated admin credential secret;
the runtime uses the `adminPassword` key from Secrets Manager, while the admin username remains fixed to `admin`.
...
For every SurvaasClusterStack there are dedicated security groups for the ECS cluster, the RDS cluster 
and the SageMaker domain.
The shared load balancer security group is owned by the root stack and reused by all cluster stacks.
...
Just fill in a unique appName. A dedicated Secrets Manager admin password is generated per SurvaasClusterStack,
and the LimeSurvey admin username is always `admin`.
...
As your load balancer directly faces the internet, Survaas provisions a starter [AWS WAF](https://docs.aws.amazon.com/waf/latest/developerguide/what-is-aws-waf.html) baseline by default.
